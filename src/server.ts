...
app.post('/api/start-chrome', async (req, res) => {
  try {
    // Caminho para o script de inicialização do Chrome (Windows)
    const scriptPath = path.join(__dirname, '../../start-chrome-debug.bat');
    
    // Verificar se o script existe
    if (!fs.existsSync(scriptPath)) {
      return res.status(500).json({ 
        success: false, 
        error: 'Script start-chrome-debug.bat não encontrado' 
      });
    }

    // Executar o script (Windows)
...
function runAutomation(configFile: string): Promise<AutomationResult> {
  return new Promise((resolve, reject) => {
    // Usar o arquivo JavaScript compilado em vez de ts-node
    const child = spawn('node', ['dist/src/automation.js', configFile], {
